...
export async function sendPasswordResetCode(email: string, code: string) {
  try {
    const mailOptions = {
      from: `"Cafetería Pantojito" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: "Recuperación de contraseña - Cafetería Pantojito",
      html: `
        <!DOCTYPE html>
        <html>
          <head>
    ...
    const info = await transporter.sendMail(mailOptions)
    console.log("[v0] Correo de recuperación enviado:", info.messageId)
    return { success: true, messageId: info.messageId }
  } catch (error) {
    console.error("[v0] Error al enviar correo de recuperación:", error)
    throw error
  }
}
